...
- `kill`: Terminate the background job identified by `command` (its `run_id`). The whole process tree is terminated, including processes the shell started (for example a dev server or a watcher spawned by the command), so a job that keeps running after its shell exits still stops. The job ends in the `killed` state and its terminal content is finalized with the output produced so far.
...
- Stop a background run: `{"type":"kill","reason":"stop the development server","command":"@temp/run/3"}` (pass the `run_id` returned by the background run)
