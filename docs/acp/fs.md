...
- **读请求**：未指定 `length` 时读到文件结尾，但单次读取上限为 **1 MiB（1,048,576 字节）**，超出返回错误；`offset` / `length` 为负数直接返回错误；`offset + length` 超出文件大小时静默截断至文件结尾；`offset ≥ 文件大小` 返回空内容。
...
- **Windows**: 仅关注 mode 的所有者写位（`0200`）。该位为 0（禁止所有者写）时把文件设为只读属性，否则设为可读写；Windows 上权限由只读属性表达，其它权限位被静默忽略。
